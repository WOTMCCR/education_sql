...
- 学习项目，全部代码可改，不保护旧架构
- 旧知识库 RAG 已退出教育问数主线；Iteration 04 删除旧 RAG 代码，后续只保留显式 `data_qa` / `meta_qa` mode
...
数据问数走 NL2SQL pipeline；数据说明走 Meta QA，不生成 SQL、不执行 SQL。数据问数回复包含：
...
| expand_search_keywords | LLM 对 query 和 seed keywords 做同义词/近义词/业务别名扩展 | query + keywords[] | expanded_keywords[] |
| recall_column | 逐关键词 Qdrant 向量搜索 → 按 column_id 去重 | expanded_keywords[] | retrieved_columns[] |
| recall_metric | 与 recall_column 对称，搜索 metric_info collection | expanded_keywords[] | retrieved_metrics[] |
| recall_value | 逐关键词 ES 全文搜索 → 按 value_id 去重 | expanded_keywords[] | retrieved_values[] |
...
1. `expand_search_keywords` 对 query 做同义词/近义词/教育业务别名扩展，生成 `expanded_keywords`
2. 合并 jieba seed 关键词 + LLM 扩展关键词，set 去重
3. 三路 recall 节点消费同一批 `expanded_keywords`
4. 逐个关键词独立搜索（不合并向量，避免语义平均化）
5. 按实体 ID 去重（同一字段可能被多个关键词通过不同路径命中）
...
### Phase 2：规则化问数 Pipeline

实现规则化最小闭环，重点关注 merge_retrieved_info 节点中的 join 路径推导、时间表达式解析、SQL 安全校验和 `DataQaResult` 包装。规则模板只作为 baseline，不代表完整泛化能力。

### Phase 3：完整 LLM NL2SQL

实现 13 节点 LangGraph DAG（§3.3）：新增 `expand_search_keywords`，并让意图结构化、候选过滤、SQL 生成、SQL 纠错真实调用 LLM。`SMOKE_STAGE=llm` 必须证明真实 LLM 节点被调用；LLM 不可用时返回结构化错误，不能用规则 baseline 冒充成功。
